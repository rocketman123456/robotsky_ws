...
static uint16_t cyber_data[4] {0};

static uint16_t s_masterid {0};
static uint16_t s_slaveid {0};
static uint8_t  s_fbtype {0};
static uint16_t s_fbid {0};

void cyber_ask_id(uint16_t id, can_frame& frame)
{
    s_slaveid     = s_masterid;
    cyber_data[0] = 0;
    cyber_data[1] = 0;
    cyber_data[2] = 0;
    cyber_data[3] = 0;
    cyber_encode(id, can_msg_type::GetId, frame);
}

void cyber_set_can_id(uint16_t id, uint16_t target_id, can_frame& frame)
{
    s_slaveid = s_masterid;
    s_slaveid |= ((uint32_t)target_id << 8);
    cyber_data[0] = 0;
    cyber_data[1] = 0;
    cyber_data[2] = 0;
    cyber_data[3] = 0;
    cyber_encode(id, can_msg_type::SetCANID, frame);
}
...
void cyber_enable(uint16_t id, can_frame& frame)
{
    ...
    cyber_data[2] = 0;
    cyber_data[3] = 0;
    cyber_encode(id, can_msg_type::EnableMotor, frame);
}
...
void cyber_disable(uint16_t id, can_frame& frame)
{
    ...
    cyber_data[2] = 0;
    cyber_data[3] = 0;
    cyber_encode(id, can_msg_type::DisableMotor, frame);
}
...
void cyber_mixed_control(uint16_t id, motor_data_t& data, can_frame& frame)
{
    ...
    cyber_data[1] = (uint16_t)((data.vel_des + 30.0) / 60.0 * 65535.0);
    cyber_data[2] = (uint16_t)(data.kp / 500.0 * 65535.0);
    cyber_data[3] = (uint16_t)(data.kd / 5.0 * 65535.0);
    cyber_encode(id, can_msg_type::MixContorl, frame);
}
...
void cyber_get_single_param(uint16_t id, uint16_t index, can_frame& frame)
{
    ...
    printf("id: %x, sid:%x, data: %x, %x, %x, %x\n", id, s_slaveid, cyber_data[0], cyber_data[1], cyber_data[2], cyber_data[3]);

    cyber_encode(id, can_msg_type::ReadParam, frame);
}
...
void cyber_encode(uint16_t id, can_msg_type type, can_frame& frame)
{
    ...
    frame.can_id |= ((uint32_t)s_slaveid) << 8;
...
void cyber_decode(const can_frame& frame)
{
    ...
    s_fbtype      = (uint8_t)((can_id << 3) >> 27);
    s_slaveid     = (uint16_t)((can_id << 8) >> 16);
    s_fbid        = (uint16_t)(can_id & 0xff);
