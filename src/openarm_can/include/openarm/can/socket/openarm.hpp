...
public:
    ...
    void init_leg_motors(const std::vector<damiao_motor::MotorType>& motor_types,
                         const std::vector<uint32_t>& send_can_ids,
                         const std::vector<uint32_t>& recv_can_ids,
                         const std::vector<damiao_motor::ControlMode>& control_modes = {});
    ...
    LegComponent& get_leg() { return *leg_; }
...
private:
    ...
    std::unique_ptr<LegComponent> leg_;
